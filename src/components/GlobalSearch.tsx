// Overlay de búsqueda global (Cmd+K). Busca en préstamos, clientes y movimientos
// en tiempo real con mínimo 2 caracteres. Abre el detalle al seleccionar un resultado.
...
interface TxWithType extends Transaction {
...
export default function GlobalSearch({ open, onClose }: GlobalSearchProps) {
  ...
  const results = useMemo(() => {
    ...
    const transactions: TxWithType[] = [
      ...state.income.map((t) => ({ ...t, txType: "income" as const })),
      ...state.expenses.map((t) => ({ ...t, txType: "expense" as const })),
    ]
      .filter((t) =>
        ...
        {total > 0 && (
          <div className="max-h-[60vh] divide-y divide-zinc-900/80 overflow-y-auto">
            {results.loans.length > 0 && (
              <div>
                <div className="px-4 py-2 text-[10px] font-medium uppercase tracking-wider text-zinc-600">Préstamos</div>
            ...
            {results.clients.length > 0 && (
              <div>
                <div className="px-4 py-2 text-[10px] font-medium uppercase tracking-wider text-zinc-600">Clientes</div>
                {results.clients.map((client) => (
                  <button key={client.id}
                    onClick={() => {
                      dispatch({ type: "SET_TAB", payload: "clients" });
                      dispatch({ type: "OPEN_MODAL", payload: { type: "client-detail", payload: { id: client.id } } });
                      onClose();
                    }}
                    className="flex w-full items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-zinc-900/60"
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {client.phone && <div className="text-[11px] text-zinc-500">{client.phone}</div>}
                    </div>
                    <ChevronRight className="h-3.5 w-3.5 shrink-0 text-zinc-700" />
                  </button>
                ))}
            ...
            {results.transactions.length > 0 && (
              <div>
                <div className="px-4 py-2 text-[10px] font-medium uppercase tracking-wider text-zinc-600">Movimientos</div>
